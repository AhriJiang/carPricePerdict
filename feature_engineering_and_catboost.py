...
def create_time_features(data):
    ...
    # 转换'regDate'和'creatDate'的日期格式, '%Y%m%d'
...
def create_additional_features(data):
    ...
    data['model_bodyType'] = data['model'].astype(str) + '_' + data['bodyType'].astype(str)
    data['brand_notRepairedDamage'] = data['brand'].astype(str) + '_' + data['notRepairedDamage'].astype(str)
    
    # power离散化
    data['power_bin'] = pd.cut(data['power'], bins=[0, 60, 120, 200, 400, float('inf')], labels=['very_low', 'low', 'medium', 'high', 'very_high'])
    
    # 统计特征
    brand_model_count = data.groupby(['brand', 'model']).size().reset_index(name='brand_model_count')
    brand_count = data.groupby('brand').size().reset_index(name='brand_count')
    brand_model_stats = brand_model_count.merge(brand_count, on='brand')
    brand_model_stats['brand_model_count_ratio'] = brand_model_stats['brand_model_count'] / brand_model_stats['brand_count']
    data = data.merge(brand_model_stats[['brand', 'model', 'brand_model_count_ratio']], on=['brand', 'model'], how='left')
    
    # 相对特征
    brand_power_mean = data.groupby('brand')['power'].mean().reset_index(name='brand_power_mean')
    data = data.merge(brand_power_mean, on='brand', how='left')
    data['power_brand_ratio'] = data['power'] / data['brand_power_mean']
    
    brand_kilometer_mean = data.groupby('brand')['kilometer'].mean().reset_index(name='brand_kilometer_mean')
    data = data.merge(brand_kilometer_mean, on='brand', how='left')
    data['kilometer_brand_ratio'] = data['kilometer'] / data['brand_kilometer_mean']
    
    # 异常值检测特征
    data['power_zscore'] = (data['power'] - data['power'].mean()) / data['power'].std()
    data['kilometer_zscore'] = (data['kilometer'] - data['kilometer'].mean()) / data['kilometer'].std()
    data['combined_outlier_score'] = data['power_zscore'].abs() + data['kilometer_zscore'].abs()
...
def feature_selection(data, categorical_cols):
    ...
    # 确保power_bin也被标记为分类特征
    if 'power_bin' not in categorical_cols and 'power_bin' in data.columns:
        categorical_cols.append('power_bin')
    
    # 添加其他交叉特征到分类特征列表中
    cross_features = ['brand_bodyType', 'brand_fuelType', 'brand_gearbox', 'model_bodyType', 'brand_notRepairedDamage']
